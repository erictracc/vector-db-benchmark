Vector Database Benchmarking: A Survey and Experimental Study
Author

Eric Traccitto
Master’s Student, Information Technology
York University, Canada
📧 [email address if you’d like to add it]
...
The objective is to evaluate the efficiency, scalability, and practicality of various vector search implementations across open-source and traditional database environments.

The study integrates five database systems and measures their performance using a unified benchmarking framework built in Python. Each system is tested using the same dataset of AI-related news articles, embedded using OpenAI’s text-embedding-3-small model.
...
Client-side (FAISS) and server-side (pgvector, Milvus, Weaviate, Qdrant) systems were benchmarked under equal conditions.
